...
func (c Client) DeviceList() (devices []Device, err error) {
	...
	for i := range lines {
		...
		serial, attrsIndex, ok := parseDeviceLine(line)
		if !ok {
		...
		devices = append(devices, Device{adbClient: c, serial: serial, attrs: mapAttrs})
	}

	return
}

func parseDeviceLine(line string) (serial string, attrsIndex int, ok bool) {
	fields := strings.Fields(line)
	if len(fields) < 2 {
		return "", 0, false
	}
	if len(fields) >= 4 && strings.Join(fields[:3], " ") == "(no serial number)" {
		return "", 4, true
	}
	if fields[0] == "" {
		return "", 0, false
	}
	return fields[0], 2, true
}
