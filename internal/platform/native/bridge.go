// Package native hosts the platform-native notification bridge inside the Go
// process. The Swift UI layer (SwiftUI/AppKit/MapKit) is linked into the Go
// binary on macOS; Windows supplies a native Toast driver. Other platforms
// use a no-op stub so the rest of the app remains headless.
package native
...
// permissionDriver is optional because macOS reports permission through the
// UserNotifications delegate, while Windows has no per-process authorization
// request for this native Toast path.
...
func newBridge(handler CommandHandler, driver uiDriver) *Bridge {
	...
	} else if permissions, ok := driver.(permissionDriver); ok {
...
// markReady lets non-callback native drivers publish the same lifecycle
// milestone as the macOS callback bridge.
...
func (b *Bridge) RequestNotificationPermission() bool {
	...
	if permissions, ok := b.driver.(permissionDriver); ok {
...
func (b *Bridge) send(event runtime.Event) {
	...
	if !started {
		...
		logger.Warn("native notification event dropped", "event", event.Type, "reason", "ui_not_running")
		return
	}
