...
type RuntimeStartRequest struct {
	...
	// IMSRegistrar 是本仓库新增接线字段（非上游代码）：注入后引擎执行真实
	// SIP REGISTER，State().IMSReady 反映注册结果；nil 时回退 Manager 注入值，
	// 仍为 nil 则沿用引擎占位（IMSReady=true）。
...
func (m *Manager) StartRuntime(ctx context.Context, req RuntimeStartRequest) (RuntimeStartResult, error) {
	...
	// IMSRegistrar 透传：请求级优先，其次 Manager 注入值（本仓库新增接线字段）。
	imsRegistrar := req.IMSRegistrar
	if imsRegistrar == nil && m != nil {
		imsRegistrar = m.imsRegistrar
	}
