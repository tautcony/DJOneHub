...
func TestManagerStartRuntimePassesIMSRegistrar(t *testing.T) {
	manager := NewManager()
	deviceID := "dev-ims"
	claim := manager.BeginStart(deviceID)
	if !claim.Accepted {
	...
	requestRegistrar := runtimehost.WireIMSRegistrar{ServerAddr: "10.0.0.1:5060"}
	var captured runtimehost.StartRequest
	manager.SetRuntimeStartForTest(func(ctx context.Context, req runtimehost.StartRequest) (*runtimehost.Instance, error) {
		captured = req
	...
	_, err := manager.StartRuntime(context.Background(), RuntimeStartRequest{
		...
		TraceID:       "trace-ims",
		Epoch:         claim.Epoch,
		Prepared:      PreparedStart{Profile: identity.Profile{IMSI: "001010000000001"}},
		IMSRegistrar:  requestRegistrar,
		Modem:         runtimeStartTestModem{},
		Dataplane:     runtimehost.DataplanePolicy{Mode: "userspace"},
		DeliveryStore: nil,
		Dispatch:      nil,
	...
	if captured.IMSRegistrar == nil {
		t.Fatal("request-level IMSRegistrar should be passed through")
	}
	reg, ok := captured.IMSRegistrar.(runtimehost.WireIMSRegistrar)
	if !ok || reg.ServerAddr != "10.0.0.1:5060" {
		t.Fatalf("captured IMSRegistrar = %#v, want request-level registrar", captured.IMSRegistrar)
	}
}

func TestManagerStartRuntimeFallsBackToConfiguredIMSRegistrar(t *testing.T) {
	manager := NewManager()
	deviceID := "dev-ims-fallback"
	claim := manager.BeginStart(deviceID)
	if !claim.Accepted {
		t.Fatalf("BeginStart() = %+v, want accepted", claim)
	}

	injected := runtimehost.WireIMSRegistrar{RegistrarURI: "sip:ims.example.com"}
	manager.ConfigureIMSRegistrar(injected)
	var captured runtimehost.StartRequest
	manager.SetRuntimeStartForTest(func(ctx context.Context, req runtimehost.StartRequest) (*runtimehost.Instance, error) {
		captured = req
		return &runtimehost.Instance{}, nil
	...
	// 请求级未携带 IMSRegistrar → 回退 Manager 注入值。
	_, err := manager.StartRuntime(context.Background(), RuntimeStartRequest{
		DeviceID:  deviceID,
		TraceID:   "trace-ims-fallback",
		Epoch:     claim.Epoch,
		Prepared:  PreparedStart{Profile: identity.Profile{IMSI: "001010000000001"}},
		Modem:     runtimeStartTestModem{},
		Dataplane: runtimehost.DataplanePolicy{Mode: "userspace"},
	...
	if captured.IMSRegistrar == nil {
		t.Fatal("configured IMSRegistrar should be passed through when request-level is nil")
	}
	reg, ok := captured.IMSRegistrar.(runtimehost.WireIMSRegistrar)
	if !ok || reg.RegistrarURI != "sip:ims.example.com" {
		t.Fatalf("captured IMSRegistrar = %#v, want injected registrar", captured.IMSRegistrar)
	}
}
