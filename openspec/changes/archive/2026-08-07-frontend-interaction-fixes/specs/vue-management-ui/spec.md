...
The frontend SHALL show, disable, or explain actions according to the capability snapshot and SHALL NOT select business behavior from the browser or the server operating-system name. This gating SHALL apply to navigation as well as in-view controls: a navigation entry that declares a `capability` the snapshot does not include SHALL be hidden, so users cannot open a view they are not permitted to use and then receive a transport error.
